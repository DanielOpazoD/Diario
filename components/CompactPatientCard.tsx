...
const CompactPatientCard: React.FC<CompactPatientCardProps> = ({ patient, onEdit, onDelete, selectionMode, selected, onToggleSelect }) => {
  ...
  return (
    <div
      className={`group relative bg-white/95 dark:bg-gray-800/90 rounded-card shadow-card border overflow-hidden transition-all duration-200 hover:shadow-elevated mb-3 ${highlightPendingPatients && pendingCount > 0 ? 'border-amber-200 dark:border-amber-700/60 ring-1 ring-amber-200/70 dark:ring-amber-600/40' : 'border-gray-200 dark:border-gray-700/50'}`}
    >
      {selectionMode && (
        <button
