  ...
  return (
    <div className="flex flex-col w-full max-w-2xl mx-auto relative group">
      ...
      <div 
        ref={scrollRef} 
        onWheel={handleWheel}
        className="flex overflow-x-auto no-scrollbar snap-x snap-mandatory py-4 w-full select-none relative z-0 outline-none focus:outline-none px-0"
      >
        {days.map((date, index) => {
          ...
          const baseClasses = isSelected 
             ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/30 scale-110 z-10 h-20 border-blue-600' 
             : isTodayDate 
                ? 'bg-amber-100 text-amber-900 dark:bg-amber-900/30 dark:text-amber-200 border-amber-200 dark:border-amber-800/50 h-16' 
          ...
          return (
            ...
            >
              ...
              <span className={`text-lg font-bold leading-none ${isSelected ? 'text-white' : isTodayDate ? 'text-amber-900 dark:text-amber-100' : 'text-gray-800 dark:text-gray-200'}`}>{format(date, 'd')}</span>
