...
const FileAttachmentManager: React.FC<FileAttachmentManagerProps> = ({
    ...
    for (let i = 0; i < totalFiles; i++) {
      ...
      try {
        if (useFirebase) {
          ...
          const attachedFile = await uploadFileForPatient(file, patientRut, patientName, token, driveFolderId);
          newAttachments.push(attachedFile);
        }
        successCount++;
  ...
  const handleDelete = async (fileId: string) => {
    ...
    // Detectar si es Firebase (usualmente no tiene long ID de Drive o el URL es de firebasestorage)
    const isFirebase = fileToDelete.driveUrl?.includes('firebasestorage.googleapis.com');
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap justify-end">
            <div
              className={`flex items-center gap-1 px-2.5 py-1 rounded-full border text-[11px] font-semibold shadow-soft ${hasDriveFolder
            ...
            <button
              ...
              disabled={isSyncing}
              className={`flex items-center gap-2 px-3 py-2 rounded-full border text-xs font-semibold transition-all shadow-sm ${isSyncing
            ...
            <button
              ...
              className={`flex items-center gap-2 px-3 py-2 rounded-full border text-xs font-semibold transition-all shadow-sm ${isCreatingFolder
